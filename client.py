...
class FtxClient:
    ...
    def get_funding_history(self,start_time: int = None, end_time: int = None,market: str = None) -> dict:
        return self._get(f'funding_rates', {'future': market,'end_time':end_time,'start_time':start_time})
    ...
    def place_conditional_order(
        self, market: str, side: str, size: float, orderType: str = 'stop',
        ...
        """
        assert orderType in ('stop', 'take_profit', 'trailing_stop')
        assert orderType not in ('stop', 'take_profit') or trigger_price is not None, \
            'Need trigger prices for stop losses and take profits'
        assert orderType not in ('trailing_stop',) or (trigger_price is None and trail_value is not None), \
            'Trailing stops need a trail value and cannot take a trigger price'

        return self._post('conditional_orders',
                          {'market': market, 'side': side, 'triggerPrice': trigger_price,
                           'size': size, 'reduceOnly': reduce_only, 'type': orderType,
